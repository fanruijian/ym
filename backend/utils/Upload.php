...
class Upload {
    ...
    private function save($file) {
        ...
        $e = mb_detect_encoding($filename, array('UTF-8', 'GBK', 'gb2312'));
        if ($e != 'UTF-8') {
            $filename = $this->autoCharset($filename, 'utf-8', strtolower($e));
        }
    ...
    public function upload($savePath ='') {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                list($width, $height) = $this->getPicSize($file['tmp_name']);
                $file['width'] = $width;
                $file['height'] = $height;
                ...
                if(function_exists($this->hashType)) {
                    $fun =  $this->hashType;
                    $e = mb_detect_encoding($file['savename'], array('UTF-8', 'GBK', 'gb2312'));
                    if ($e != 'UTF-8') {
                        $file['savename'] = $this->autoCharset($file['savename'], 'utf-8', strtolower($e));
                    }
    ...
    /**
     * 获取上传文件的尺寸(确保上传为图片时使用此方法）
     */
    private function getPicSize($filename) {
    ...
    private function getSaveName($filename) {
		if (isset($this->config['saveName'])) {
            $e = mb_detect_encoding($this->config['saveName'], array('UTF-8', 'GBK', 'gb2312'));
            if ($e == 'UTF-8') return $this->config['saveName'];
			return $this->autoCharset($this->config['saveName']);
		}
        $rule = $this->saveRule;
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            $saveName = $this->autoCharset($filename['name']);
        } else {
            $rules = explode('_', $rule);
            $nameParts = [];
            list($width, $height) = $this->getPicSize($filename['tmp_name']);
